...
Dygraph.prototype.mouseMove_ = function(event) {
  ...
  if (this.selPoints_.length > 0) {
    ...
    // EDITED THIS CODE (andrewharvey)
    // Set the status message to indicate the selected point(s)
    var replace = "<span class=\"legendDate\">" + this.attr_('xValueFormatter')(lastx, this) + ":" + "</span>";
...
//NEW FUNCTION (andrewharvey)
/**
 * Convert a number in hours to hour:minute time format.
 * @param {Number} num The floating point number of hours to convert
 * @return {String} The string in hour:minute time format
 * @private
 */
Dygraph.prototype.formatHour_ = function(num) {
  //24hr time
  //return Math.floor(num) + ":" + Math.round( ((num - Math.floor(num)) * 60) * 2 )/2;
  //12hr time (if hour == 0, should print as 12. eg 00:20 is 12:20am)
  var suffix = "";
  if (Math.floor(num) >= 12) {
    suffix = " pm";
    num = num - 12;
  }else {
    suffix = " am";
  ...
  var minutes = ((num - Math.floor(num)) * 60).toFixed();
  if (minutes < 10)
    minutes = "0" + minutes;
  return Math.min(Math.floor(num),12) + ":" + minutes + suffix;
};
